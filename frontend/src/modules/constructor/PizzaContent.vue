<template>
	<div class="content__constructor">
		<div 
			...
			:class="`pizza pizza--foundation--${modelValue.dough}-${modelValue.sauce}`"
		>
...
const ingredientClasses = computed(() => {
	const result = [];
	for (let key in props.modelValue.ingredients) {
		if (props.modelValue.ingredients[key] == 1) {
			result.push(`pizza__filling--${key}`);
		}
		if (props.modelValue.ingredients[key] == 2) {
			result.push(`pizza__filling--${key} pizza__filling--second`);
		}
		if (props.modelValue.ingredients[key] == 3) {
			result.push(`pizza__filling--${key} pizza__filling--third`);
		}
	}
	return result;
})
